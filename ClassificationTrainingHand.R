# The four parameters that can be adjusted for the file. 
# Commented out arguments are for the .R file to be used with the file.
  args = commandArgs(TRUE)
  nFst <- args[1]
  minReads <- args[2]
  svmC <- args[3]
  specificsample <- args[4]

# svmC = 1000
# minReads = 250
# nFst = 10
# specificsample = "S007-Hp-T1-R2"
...
read_one_file <- function(file) { 
  ...
  suppressMessages(read_table2(
    "MarkersForTargetedPanel_012021.txt"
...
# read_four_files takes four file names and selects SNPs
#
...
read_four_files <- function(ml, topNum = nFst, readmin = minReads) {
  ...
  expand(
    top10, MarkerPosition, IndComparison
  ) -> x
...
#print(read_four_files(ml[[1]], nFst, minReads ))
